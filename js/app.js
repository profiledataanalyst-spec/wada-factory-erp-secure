(() => {
  ...
  function importDuplicateKey(projectName,itemName,site,bomNumber,jobNumber,stage,quantity){return[projectName,itemName,site,bomNumber,jobNumber,stage,quantity].map(x=>String(x??'').trim().toLowerCase()).join('|');}
  ...
  function validateImportRows(rows){
    ...
    rows.forEach((r,index)=>{
      ...
      const rec={
        sourceRow:index+2,projectName,itemName,site:String(getCol(r,'SITE DETAILS')).trim(),
      ...
      const key=importDuplicateKey(projectName,itemName,rec.site,rec.bomNumber,rec.jobNumber,stage,qty);
      if(seen.has(key))rec.errors.push('Exact duplicate row exists in this file');
      seen.add(key);
      const existing=state.items.some(i=>importDuplicateKey(projectById(i.projectId)?.name,i.itemName,i.site,i.bomNumber,i.jobNumber,STAGES[i.currentStage],i.quantity)===key);
  ...
  function renderImportPreview(){
    ...
    result.innerHTML=`<div class="validation-summary"><div class="validation-box"><strong>${r.total}</strong><span>Total Rows</span></div><div class="validation-box"><strong class="text-success">${r.valid.length}</strong><span>Valid Rows</span></div><div class="validation-box"><strong class="text-warning">${r.warnings.length}</strong><span>Warnings</span></div><div class="validation-box"><strong class="text-danger">${r.failed.length}</strong><span>Failed Rows</span></div><div class="validation-box"><strong>${new Set(r.valid.map(x=>x.projectName)).size}</strong><span>Projects</span></div></div>
      ...
      <div class="table-wrap"><table><thead><tr><th>Row</th><th>Project</th><th>Item</th><th>Qty</th><th>BOM / Job</th><th>Stage</th><th>Validation</th></tr></thead><tbody>${[...r.valid.slice(0,80),...r.failed.slice(0,40)].map(x=>`<tr><td>${x.sourceRow}</td><td>${esc(x.projectName||'—')}</td><td>${esc(x.itemName||'—')}<div class="small muted">${esc(x.site)}</div></td><td>${fmtNumber(x.quantity)}<div class="small muted">${esc(x.quantitySource)}</div></td><td>${esc(x.bomNumber||'—')}<div class="small muted">${esc(x.jobNumber||'—')}</div></td><td>${esc(x.stage||x.statusRaw||'—')}</td><td>${x.errors.length?`<span class="text-danger small">${esc(x.errors.join('; '))}</span>`:x.warnings.length?`<span class="text-warning small">${esc(x.warnings.join('; '))}</span>`:'<span class="text-success small">Valid</span>'}</td></tr>`).join('')}</tbody></table></div>
